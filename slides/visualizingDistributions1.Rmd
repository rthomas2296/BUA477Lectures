...
h1
```
#### Notice the dip around 10. You don't want to lose that.
]
...
### Look at the next screen of plots
- A bin width of 1 is too "fine".  You see too many variations
- A bin width of 3 or 5 seems good.
- A bin width of 15 seem too large a bin width. Notice, we have lost the dip at 10.
...
###### On the next screen you are going to see different plots
###### They have been done with different bandwidths, and in one case, a different kernel
###### There are many kernels for doing density estimates. The ones shown here are Gaussian and Rectangular
- ggplot(titanic, aes(x = age)) + geom_density(fill = "skyblue", bw = .5, kernel = "gaussian")
- ggplot(titanic, aes(x = age)) + geom_density(fill = "skyblue", bw = 2.0, kernel = "gaussian")
- ggplot(titanic, aes(x = age)) + geom_density(fill = "skyblue", bw = 1, kernel = "rectangular")
...
```
Is the female bar over the male bar?
]
...
```
Now we have a 3rd color, and do the bars really start at 0?
]
...
```
When you have exactly two distributions.  Reverse the direction of one.
]
