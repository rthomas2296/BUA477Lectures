---
title: "BUA 477 Visual Analytics"
...
```
---
On the next screen, you are going to see four map projections — different ways of flattening a sphere onto a plane, each trading off a different kind of distortion.

Cartesian -- Longitude and latitude plotted directly as x/y on a rectangular grid. Simple and easy to compute, but badly stretches areas near the poles (Greenland looks enormous).

Interrupted Goode homolosine — An equal-area projection that's "interrupted" (cut into lobes) over the oceans so landmasses keep accurate size and shape. Great for showing land-based data; useless for showing oceans or continuous routes.

Robinson — A compromise projection: neither equal-area nor conformal, tuned to just look right. Was the National Geographic standard from 1988–1998.

Winkel tripel — Another compromise, mathematically averaging two different projections. Minimizes area, direction, and distance distortion simultaneously ("tripel" = three). Replaced Robinson as National Geographic's standard in 1998.

The main lesson: no flat map can preserve area, shape, distance, and direction at once. You pick which error you can live with.
...
To check version of RStudio:
- Launch RStudio - Go to Help --> About RStudio

To check version of R
- Check the version of R that is reported at the top of the console (bottom right pane)
...
and *italics*, and ***bolded italics***.
```
]
...
.small-font[
This is a sentence in Markdown, containing `code`, **bold text**, and *italics*, and ***bolded italics***.
]
