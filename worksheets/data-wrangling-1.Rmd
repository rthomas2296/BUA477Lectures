...
```{r filter-species-island-hint, eval=FALSE}
penguins |>
...
```{r select-exercise-hint, eval=FALSE}
penguins |>
  select(___)
```
...
```{r select-remove-hint, eval=FALSE}
penguins |>
  select(___)
```
...
```{r select-remove2-hint, eval=FALSE}
penguins |>
  select(-___, -___)
```
...
```{r arrange-bill-length-hint, eval=FALSE}
penguins |>
  arrange(___)
```
...
```{r arrange-bill-length-desc-hint-1, eval=FALSE}
penguins |>
  arrange(___)
```

```{r arrange-bill-length-desc-hint-2, eval=FALSE}
penguins |>
...
```{r count-simple-hint, eval=FALSE}
penguins |>
  count(___)
```
...
```{r count-complex-hint, eval=FALSE}
penguins |>
  count(___, sex)
```
...
```{r count-complex2-hint, eval=FALSE}
penguins |>
  count(___, species)
```
...
```{r analysis-chain-hint-1, eval=FALSE}
penguins |>
  count(___) |>
  arrange(___)
```

```{r analysis-chain-hint-2, eval=FALSE}
penguins |>
  count(species) |>
  arrange(___)
```
...
```{r analysis-chain2-hint, eval=FALSE}
penguins |>
  count(species) |>
  arrange(desc(___))
```
...
```{r mutate-bill-ratio, exercise=TRUE, eval=FALSE}
penguins |>
...
```{r mutate-bill-ratio-hint, eval=FALSE}
penguins |>
  select(species, bill_length_mm, bill_depth_mm) |>
  mutate(
    bill_ratio = ___
...
```{r flipper-length-count, exercise = TRUE, eval=FALSE}
penguins |>
  mutate(
    ___
  ) |>
...
```{r flipper-length-count-hint-1, eval=FALSE}
penguins |>
  mutate(
...
```{r flipper-length-count-hint-2, eval=FALSE}
penguins |>
  mutate(
    flipper_length = ifelse(___, "greater than 200mm", "less than 200mm")
  ) |>
